...
header("Content-type: text/html; charset=utf-8");	# send utf-8 content encoding header
...
if(isset($_GET['out'])) $_GET['link'] = $_GET['out'];
...
# if(remote_ip() !== "89.133.175.240") { die("Maintenance. Will be back soon."/*"Sorry, this blog is still in a closed state. You'll see when it's open."*/); }
...
#echo '<!--'; var_dump($mb_search); echo '-->';
/*$str = "<ul>\n";
$res = query("select post_id, post_title from vblog_posts order by post_date desc limit 10");
for($i = 0; $i < 10; $i++) {
	$row = mysql_fetch_row($res);
	$str .= "<li><a href=\"/post/".$row[0]."/".format($row[1],'url')."/\">".$row[1]."</a></li>\n";
}
echo $str."</ul>";*/
